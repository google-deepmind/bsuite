...
class ActorCritic(base.Agent):
  ...
  def __init__(
    ...
    initial_params = init(
        next(rng), jnp.zeros((1, *obs_spec.shape), dtype=jnp.float32))
    ...
    def loss(trajectory: sequence.Trajectory):
      """"Actor-critic loss."""
  ...
  def update(self, timestep: dm_env.TimeStep, action: base.Action,
    ...
    if self._buffer.full() or new_timestep.last():
      ...
      self._state = self._update(self._state, trajectory)


def default_agent(obs_spec: specs.Array,
                  action_spec: specs.DiscreteArray) -> base.Agent:
  """Creates an actor-critic agent with defautl hyperparameters."""

  def network(inputs: jnp.float32) -> Tuple[Logits, Value]:
