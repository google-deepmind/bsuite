...
from bsuite.experiments.mdp_playground_r_sparse import sweep
...
def score(df: pd.DataFrame, scaling_var='reward_density') -> float:
...
def plot_average(df: pd.DataFrame,
                 sweep_vars: Sequence[str] = None,
                 group_col: str = 'reward_density') -> gg.ggplot:
  """Plots the average regret through time by reward_density."""
  df = mdp_playground_analysis.mdpp_preprocess(df)
